...
LRESULT HandleWMKeyDown(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (wParam)
    {
        ...
        case VK_F3:
            ...
            {
                // 获取当前选中的项目
                INT_PTR currentSel = ListView_GetNextItem(g_hListView, -1, LVNI_SELECTED);
                if (currentSel != -1)
                {
                    // 跳过提示行
                    INT_PTR hintCount = GetHintRowCount();
                    if (currentSel >= hintCount)
                    {
                        // 转换为实际快捷方式索引
                        INT_PTR shortcutIndex = currentSel - hintCount;
                        
                        // 检查索引是否有效
                        if (shortcutIndex >= 0 && shortcutIndex < (INT_PTR)g_searchResults.size())
                        {
                            LogToFile("WM_KEYDOWN: F3键按下，编辑快捷方式");
            ...
            if (g_hListView && IsWindow(g_hListView))
            {
                ...
                if (currentSel != -1)
                {
                    ...
                    if (currentSel >= hintCount)
                    {
                        ...
                        if (shortcutIndex >= 0 && shortcutIndex < (INT_PTR)g_searchResults.size())
                        {
                            LogToFile("WM_KEYDOWN: Delete键按下，删除快捷方式");
                            ...
                            {
                                // 从搜索结果中移除
                                if (shortcutIndex < (INT_PTR)g_searchResults.size())
                                {
                                    // 找到对应的原始快捷方式并移除
                                    const ShortcutItem& selectedItem = g_searchResults[shortcutIndex];
                                    for (auto it = g_shortcuts.begin(); it != g_shortcuts.end(); ++it)
                                    {
                                        // 使用字符串比较函数来比较数组
                                        if (wcscmp(it->name, selectedItem.name) == 0 && 
                                            wcscmp(it->path, selectedItem.path) == 0)
                                        {
                                            g_shortcuts.erase(it);
                                            break;
                                        }
                                    }
